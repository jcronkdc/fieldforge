...
export function createAIRouter(): Router {
  ...
  router.post('/predictive-maintenance', async (req: Request, res: Response) => {
    try {
  ...
  router.get('/weather/:zipCode', async (req: Request, res: Response) => {
    try {
      ...
      const weather = await getCurrentWeather(zipCode, country as string);
      res.json(weather);
    } catch (error) {
  ...
  router.get('/weather/:zipCode/forecast', async (req: Request, res: Response) => {
    try {
      const { zipCode } = req.params;
      ...
      const forecast = await getWeatherForecast(zipCode, country as string, parseInt(days as string));
      res.json(forecast);
    } catch (error) {
  ...
  router.get('/weather/project/:projectId', async (req: Request, res: Response) => {
    try {
      ...
      const weatherData = await getProjectWeather(projectId, forecast === 'true');
  ...
  router.post('/weather/analysis', async (req: Request, res: Response) => {
    try {
      ...
      const analysis = await analyzeWeatherImpact(zipCode, projectId, activities);
 ...
 */
async function gatherContextData(context: any): Promise<any> {
  const data: any = {
 ...
 */
function buildConstructionExpertPrompt(): string {
 ...
 */
function buildContextualUserMessage(userQuery: string, contextData: any): string {
 ...
 */
function parseAIResponse(aiContent: string, originalQuery: string, context: any): any {
 ...
 */
function extractActionableItems(content: string): string[] {
 ...
 */
function generateIntelligentFallback(content: string, context: any): any {
 ...
 */
async function analyzeDocumentWithAI(documentUrl: string, documentType: string, analysisType: string, projectId?: string): Promise<any> {
 ...
 */
function buildDocumentAnalysisPrompt(documentType: string, analysisType: string): string {
 ...
 */
async function analyzeImageWithAI(imageUrl: string, analysisType: string, projectId?: string): Promise<any> {
 ...
 */
function buildImageAnalysisPrompt(analysisType: string): string {
  if (analysisType === 'safety') {
 ...
 */
function extractDetectedItems(analysis: string): string[] {
  const items: string[] = [];
 ...
 */
async function performRiskAnalysis(projectId: string): Promise<any> {
  try {
 ...
 */
async function gatherProjectRiskData(projectId: string): Promise<any> {
  const data: any = {
    project: {},
    safety: {},
    schedule: {},
 ...
 */
function extractRisks(analysis: string): any[] {
  const risks: any[] = [];
 ...
 * Fallback risk analysis
 */
function generateFallbackRiskAnalysis(projectData: any): any {
  const risks: any[] = [];
 ...
 */
async function predictMaintenanceNeeds(projectId: string, equipmentId?: string): Promise<any> {
  try {
    // Get equipment data
    let equipmentQuery = `
    ...
    const result = await query(equipmentQuery, params);
 ...
 */
function generateFallbackMaintenancePredictions(equipmentData: any[]): any {
  const predictions: any[] = [];
  const now = new Date();
