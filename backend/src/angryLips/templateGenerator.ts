export type TemplateLength = "quick" | "classic" | "epic";
...
export function generateTemplate(options: TemplateOptions): GeneratedTemplate {
  ...
  if (options.length !== "quick") {
    segments.push(
      `After the reveal, we ${verbIng} toward the ${place} again, chanting ${sillyWord} in perfect ${adverb} rhythm.`
    );
  }

  if (options.length === "epic") {
    segments.push(
      `By the time the ${vehicle} lifts off, ${relative} is humming along with the ${sound} while ${celebrity} streams the victory.`
    );
  }
  ...
  const tagCounts = blanks.reduce<Record<string, number>>((acc, blank) => {
...
function buildBlankCandidates(): BlankCandidate[] {
