...
export interface AIResponse {
  ...
  model: string;
  tokens?: number;
  provider: 'local' | 'claude' | 'openai' | 'grok' | 'fallback';
 ...
 */
export async function shouldUseLocalAI(companyId?: string, userId?: string): Promise<boolean> {
 ...
 */
export async function getLocalAIConfig(companyId?: string) {
  if (companyId) {
    const result = await query(`
...
export async function callLocalAI(
  ...
  try {
    ...
    try {
      ...
      const response = await axios.post(`${config.url}/v1/chat/completions`, {
        model: config.model,
        messages: messages,
        temperature: 0.7,
        max_tokens: 2000
 ...
 */
export async function callCloudAI(messages: AIMessage[]): Promise<AIResponse> {
