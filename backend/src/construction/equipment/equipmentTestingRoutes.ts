...
export function createEquipmentTestingRouter(): Router {
  const router = Router();
  ...
  router.post('/', async (req: Request, res: Response) => {
    try {
      ...
      const equipmentCheck = await query(
        'SELECT id, name, equipment_code FROM equipment WHERE id = $1 AND company_id = $2',
        [equipment_id, companyId]
      );
      ...
      if (measurements && Array.isArray(measurements)) {
        measurements.forEach((m: any) => {
          if (m.min_acceptable !== null && m.value < m.min_acceptable) {
            ...
            (m.min_acceptable !== null && m.value < m.min_acceptable * 1.1) ||
      ...
      if (status === 'pass' || status === 'warning') {
        ...
        const nextDays = frequencyDays[testFreq as keyof typeof frequencyDays] || 30;
        ...
        await query(
          ...
          SET next_test_due = $1 
          WHERE id = $2`,
          [nextDate.toISOString(), result.rows[0].id]
        );
      }
