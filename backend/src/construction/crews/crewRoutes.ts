...
export function createCrewRouter(): Router {
  const router = Router();
        ...
        if (lead_user_id) {
          await query(
            `INSERT INTO crew_members 
             ...
             VALUES ($1, $2, 'lead', NOW(), 'active')`,
            [crew.id, lead_user_id]
          );
        }
  ...
  router.get('/:id/members', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  // ADD member to crew
  router.post('/:id/members', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      const existing = await query(
        'SELECT id FROM crew_members WHERE crew_id = $1 AND user_id = $2 AND status = $3',
        [id, user_id, 'active']
      );
  ...
  router.get('/:id/availability', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { date_from = new Date(), date_to } = req.query;

      const endDate = date_to || new Date(new Date(date_from as string).getTime() + 30 * 24 * 60 * 60 * 1000);
      ...
      const result = await query(availabilityQuery, [id, date_from, endDate]);

      // Calculate availability percentages
      const totalMembers = await query(
        'SELECT COUNT(*) as total FROM crew_members WHERE crew_id = $1 AND status = $2',
        [id, 'active']
      );
  ...
  router.get('/:id/certifications', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      const result = await query(certQuery, [id]);
  ...
  router.post('/:id/assign', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      try {
        ...
        await query(
          `INSERT INTO project_schedule
           ...
           VALUES ($1, $2, $3, $4, $5)`,
          [project_id, id, start_date, end_date || null, req.user?.id]
        );
  ...
  router.put('/:id', authenticateRequest, async (req: Request, res: Response) => {
    try {
      ...
      const result = await query(
        `UPDATE crews SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
        values
      );
