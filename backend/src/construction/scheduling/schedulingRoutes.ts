...
export function createSchedulingRouter(): Router {
  ...
  router.post('/lookahead', async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `INSERT INTO lookahead_activities (
  ...
  router.delete('/lookahead/:id', async (req, res) => {
    try {
      ...
      const result = await pool.query(
        'DELETE FROM lookahead_activities WHERE id = $1 RETURNING activity_name',
        [id]
      );
  ...
  router.post('/lookahead/:id/dependencies', async (req, res) => {
    try {
      ...
      const result = await pool.query(
        ...
        RETURNING *`,
        [id, depends_on_id, dependency_type || 'finish_to_start', lag_days || 0]
      );
  ...
  router.get('/conflicts', async (req, res) => {
    try {
      ...
      const result = await pool.query(
        ...
        ORDER BY scheduled_date, location`,
        [start_date, end_date]
      );
