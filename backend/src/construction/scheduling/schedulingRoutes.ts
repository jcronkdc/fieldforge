...
export function createSchedulingRouter(): Router {
  ...
  router.get('/projects/:projectId/schedule', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { projectId } = req.params;
      
      // Get project details for date range
      const projectResult = await query(
        'SELECT name, start_date, end_date FROM projects WHERE id = $1',
        [projectId]
      );
      ...
      const tasksResult = await query(
        ...
        ORDER BY st.start_date, st.sort_order`,
        [projectId]
      );
      ...
      res.json({
        ...
        stats: {
          ...
          overdue_tasks: tasksResult.rows.filter(t => 
  ...
  router.post('/tasks', authenticateRequest, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const taskResult = await query(
          `INSERT INTO schedule_tasks 
           ...
           RETURNING *`,
          [project_id, parent_task_id, name, description, task_type || 'task',
        ...
        if (assigned_crews && Array.isArray(assigned_crews)) {
          for (const crewId of assigned_crews) {
            await query(
              `INSERT INTO task_assignments (task_id, crew_id, assigned_by)
               VALUES ($1, $2, $3)`,
              [task.id, crewId, userId]
            );
          }
        }
  ...
  router.put('/tasks/:taskId/progress', authenticateRequest, async (req: Request, res: Response) => {
    try {
      ...
      const result = await query(
        `UPDATE schedule_tasks 
         ...
         WHERE id = $4
         RETURNING *`,
        [progress_percentage, status, notes, taskId]
      );
  ...
  router.get('/lookahead/:projectId', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { projectId } = req.params;
  ...
  // CREATE milestone
  router.post('/milestones', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const userId = req.user?.id;
      const {
        project_id,
      ...
      const result = await query(
        `INSERT INTO project_milestones 
         ...
         RETURNING *`,
        [project_id, name, description, milestone_date, 
  ...
  router.get('/resources/:projectId', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { projectId } = req.params;
      ...
      const crewResult = await query(
        ...
        ORDER BY c.name`,
        start_date && end_date ? [projectId, start_date, end_date] :
  ...
  router.put('/tasks/:taskId/reschedule', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { taskId } = req.params;
      ...
      try {
        ...
        const daysDiff = Math.floor(
          (new Date(start_date).getTime() - new Date(task.start_date).getTime()) / 
          (1000 * 60 * 60 * 24)
        );
  ...
  router.get('/conflicts/:projectId', authenticateRequest, async (req: Request, res: Response) => {
    try {
      const { projectId } = req.params;
      
      // Find crew conflicts (double-booked)
      const crewConflicts = await query(
        `WITH crew_tasks AS (
          SELECT 
            ta.crew_id,
            c.name as crew_name,
        ...
        AND ct1.end_date >= ct2.start_date`,
        [projectId]
      );
      
      // Find dependency conflicts (task starts before dependency completes)
      const dependencyConflicts = await query(
        `SELECT 
...
function calculateUtilization(resources: any[]): number {
