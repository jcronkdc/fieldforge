...
const readingSchema = z.object({
  metric_type: z.enum(['air_quality', 'noise_level', 'dust_level', 'water_quality', 'temperature', 'humidity']),
  value: z.number(),
  unit: z.string(),
  location: z.string(),
...
export function createEnvironmentalRouter(): Router {
  const router = Router();
  ...
  router.post('/readings', async (req: Request, res: Response) => {
    try {
      ...
      let status = 'normal';
      if (validatedData.metric_type === 'air_quality' && validatedData.value > 100) status = 'violation';
      else if (validatedData.metric_type === 'air_quality' && validatedData.value > 50) status = 'warning';
      else if (validatedData.metric_type === 'noise_level' && validatedData.value > 85) status = 'violation';
      else if (validatedData.metric_type === 'noise_level' && validatedData.value > 75) status = 'warning';
      else if (validatedData.metric_type === 'dust_level' && validatedData.value > 0.15) status = 'violation';
      else if (validatedData.metric_type === 'dust_level' && validatedData.value > 0.1) status = 'warning';
      ...
      auditLogger.log('environmental_reading_created', userId, {
  ...
  router.post('/incidents', async (req: Request, res: Response) => {
    try {
      ...
      const result = await query(
        `INSERT INTO environmental_incidents 
      ...
      auditLogger.log('environmental_incident_created', userId, {
  ...
  router.put('/incidents/:id', async (req: Request, res: Response) => {
    try {
      ...
      const result = await query(
        `UPDATE environmental_incidents
         SET status = $1, 
         ...
         WHERE id = $4
         RETURNING *`,
        [status, corrective_actions || null, resolution_notes || null, id]
      );
      ...
      auditLogger.log('environmental_incident_updated', userId, {
        incident_id: id,
        status,
        corrective_actions
      });
  ...
  router.get('/reports/compliance', async (req: Request, res: Response) => {
    try {
      ...
      const readingsResult = await query(
        ...
        [project_id || null, start_date || null, end_date || null]
      );
