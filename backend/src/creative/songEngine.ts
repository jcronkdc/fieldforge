...
export async function convertToSong(data: {
  userId: string;
  storyContent: string;
  title: string;
  genre?: string;
  ...
  const canUse = await checkUserAITier(data.userId, 'song');
  if (!canUse) {
...
Return as JSON:
{
  ...
  // Consume tokens
  await consumeAITokens(data.userId, 'song', 3000);
...
// Analyze rhyme scheme
function analyzeRhymeScheme(structure: any): string {
  const schemes: string[] = [];
  
  for (const section in structure) {
    const lines = structure[section];
    if (Array.isArray(lines) && lines.length > 1) {
      const endWords = lines.map((line: string) => {
...
function analyzeMeter(structure: any): string {
  const patterns: string[] = [];
  
  for (const section in structure) {
    const lines = structure[section];
    if (Array.isArray(lines)) {
      const syllableCounts = lines.map((line: string) => {
...
export function formatSongText(song: SongProject): string {
...
export async function addCollaborator(data: {
  ...
  role: 'lyricist' | 'composer' | 'producer' | 'vocalist' | 'instrumentalist';
  contributionPercentage?: number;
}) {
  const result = await query(
    `INSERT INTO song_collaborations (
    ...
    RETURNING *`,
    [data.songId, data.collaboratorId, data.role, data.contributionPercentage || 0]
  );
