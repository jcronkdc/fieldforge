...
function createReportingRouter() {
    ...
    router.get('/templates', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const templates = [
    ...
    // GENERATE report
    router.post('/generate', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const companyId = req.user?.company_id || req.headers['x-company-id'];
            const { template_id, project_id, date_from, date_to, include_photos, include_signatures, custom_data } = req.body;
            ...
            const reportResult = await (0, database_js_1.query)(`INSERT INTO reports 
         (template_id, project_id, company_id, generated_by, 
          date_from, date_to, status, report_data)
         VALUES ($1, $2, $3, $4, $5, $6, 'generated', $7)
         RETURNING *`, [template_id, project_id, companyId, userId,
            ...
            // Save PDF
            await (0, database_js_1.query)(`UPDATE reports 
         SET pdf_data = $1, file_size = $2 
         WHERE id = $3`, [Buffer.from(pdfContent, 'base64'), pdfContent.length, report.id]);
            res.status(201).json({
                id: report.id,
    ...
    // DOWNLOAD report PDF
    router.get('/download/:reportId', auth_js_1.authenticateRequest, async (req, res) => {
        try {
    ...
    // GET report history
    router.get('/history', auth_js_1.authenticateRequest, async (req, res) => {
        try {
    ...
    router.post('/schedule', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { template_id, project_id, frequency, day_of_week, time_of_day, recipients } = req.body;
            if (!template_id || !project_id || !frequency || !recipients) {
                return res.status(400).json({
                    error: 'Missing required fields'
                });
            }
            const result = await (0, database_js_1.query)(`INSERT INTO report_schedules 
         (template_id, project_id, frequency, day_of_week, 
          time_of_day, recipients, created_by, active)
         VALUES ($1, $2, $3, $4, $5, $6, $7, true)
         RETURNING *`, [template_id, project_id, frequency, day_of_week,
    ...
    router.get('/executive-dashboard/:projectId', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const { projectId } = req.params;
            const [projectMetrics, safetyMetrics, qualityMetrics, scheduleMetrics, budgetMetrics] = await Promise.all([
                // Project overview
...
async function generateDailyReport(projectId, date) {
    const [activities, safety, weather, crews] = await Promise.all([
        (0, database_js_1.query)(`
      SELECT * FROM daily_reports 
      WHERE project_id = $1 AND date = $2`, [projectId, date]),
        (0, database_js_1.query)(`
      SELECT * FROM safety_incidents 
      WHERE project_id = $1 AND DATE(created_at) = $2`, [projectId, date]),
        (0, database_js_1.query)(`
      ...
      GROUP BY c.name`, [projectId, date])
    ]);
    return {
        date,
