...
exports.createSafetyRouter = createSafetyRouter;
const express_1 = require("express");
const auth_js_1 = require("../../middleware/auth.js");
const database_js_1 = require("../../database.js");
const auditLog_js_1 = require("../../middleware/auditLog.js");
function createSafetyRouter() {
    const router = (0, express_1.Router)();
    // ============================================================================
    // SAFETY MANAGEMENT ENDPOINTS - COMPLETE E2E PATHWAYS
    // ============================================================================
    // CREATE safety incident
    router.post('/incidents', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            ...
            const { type, severity, location_description, description, project_id, immediate_actions, witnesses } = req.body;
            ...
            try {
                const incidentResult = await (0, database_js_1.query)(`INSERT INTO safety_incidents 
           (type, severity, location_description, description, project_id, 
            immediate_actions, witnesses, reported_by, company_id, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'open')
           RETURNING *`, [type, severity, location_description, description, project_id,
    ...
    router.get('/incidents', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const companyId = req.user?.company_id || req.headers['x-company-id'];
    ...
    router.post('/briefings', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const companyId = req.user?.company_id || req.headers['x-company-id'];
            if (!userId) {
            ...
            const { project_id, topics, hazards_identified, safety_reminders, attendees, date } = req.body;
            // Validate required fields
            ...
            try {
                // Create briefing
                const briefingResult = await (0, database_js_1.query)(`INSERT INTO safety_briefings
           (project_id, conducted_by, topics, hazards_identified, 
            safety_reminders, date, company_id)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           RETURNING *`, [project_id, userId, topics, hazards_identified,
                    safety_reminders, date || new Date(), companyId]);
                const briefing = briefingResult.rows[0];
                // Record attendee signatures
                for (const attendee of attendees) {
                    await (0, database_js_1.query)(`INSERT INTO safety_briefing_attendees
             (briefing_id, user_id, signature_data, signed_at)
             VALUES ($1, $2, $3, NOW())`, [briefing.id, attendee.user_id, attendee.signature]);
                }
                await (0, database_js_1.query)('COMMIT');
    ...
    router.get('/metrics', auth_js_1.authenticateRequest, async (req, res) => {
        try {
          ...
          WHERE company_id = $1 
          AND date >= NOW() - INTERVAL '7 days'`, [companyId]),
                // Active permits
                (0, database_js_1.query)(`
    ...
    // CREATE work permit
    router.post('/permits', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const companyId = req.user?.company_id || req.headers['x-company-id'];
            const { type, project_id, work_description, hazards, controls, valid_until, authorized_workers } = req.body;
            const permitResult = await (0, database_js_1.query)(`INSERT INTO work_permits
         (type, project_id, work_description, hazards, controls,
          valid_until, created_by, authorized_workers, company_id, status)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'active')
         RETURNING *`, [type, project_id, work_description, hazards, controls,
    ...
    router.put('/incidents/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const { id } = req.params;
            const { status, investigation_notes, corrective_actions } = req.body;
            const result = await (0, database_js_1.query)(`UPDATE safety_incidents
         SET status = $1, 
             investigation_notes = $2, 
             corrective_actions = $3,
             updated_at = NOW()
         WHERE id = $4
         RETURNING *`, [status, investigation_notes, corrective_actions, id]);
    ...
    router.get('/permits', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const companyId = req.user?.company_id || req.headers['x-company-id'];
    ...
    router.post('/permits', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const companyId = req.user?.company_id || req.headers['x-company-id'];
            const { type, project_id, work_description, hazards, controls, valid_start, valid_until, authorized_workers, special_requirements } = req.body;
            // Validate required fields
    ...
    // APPROVE work permit
    router.put('/permits/:id/approve', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            const result = await (0, database_js_1.query)(`UPDATE work_permits
         SET status = 'active',
             approved_by = $1,
             approved_at = NOW(),
             updated_at = NOW()
    ...
    // CANCEL work permit
    router.put('/permits/:id/cancel', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
         ...
         RETURNING *`, [reason || 'Cancelled by user', userId, id]);
            if (result.rowCount === 0) {
