...
function createTestingRouter() {
    ...
    router.post('/tests', async (req, res) => {
        try {
            ...
            const { equipment_id, test_type, test_name, scheduled_date, compliance_standard, template_id } = req.body;
            ...
            try {
                ...
                const equipmentResult = await (0, database_js_1.query)('SELECT name, equipment_type FROM equipment WHERE id = $1', [equipment_id]);
    ...
    router.put('/tests/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const { status, completed_date, results, notes, performed_by } = req.body;
            const result = await (0, database_js_1.query)(`UPDATE equipment_tests 
        SET 
        ...
        RETURNING *`, [
            ...
            if (status === 'passed' && completed_date) {
                ...
                const templateResult = await (0, database_js_1.query)(`SELECT frequency_days FROM test_templates 
          WHERE equipment_type = $1 AND test_type = $2 
          LIMIT 1`, [test.equipment_type, test.test_type]);
                if (templateResult.rows.length > 0) {
                    const frequencyDays = templateResult.rows[0].frequency_days;
                    const nextDate = new Date(completed_date);
                    nextDate.setDate(nextDate.getDate() + frequencyDays);
                    await (0, database_js_1.query)(`INSERT INTO equipment_tests (
              equipment_id, equipment_name, equipment_type,
              test_type, test_name, scheduled_date,
              status, compliance_standard, created_by
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
    ...
    router.get('/analytics', async (req, res) => {
      ...
      `;
            const metricsParams = [companyId];
            if (start_date)
                metricsParams.push(start_date);
            if (end_date)
                metricsParams.push(end_date);
            const metricsResult = await (0, database_js_1.query)(metricsQuery, metricsParams);
        ...
        ORDER BY total DESC
      `;
            const typeResult = await (0, database_js_1.query)(typeQuery, metricsParams);
            res.json({
                success: true,
                data: {
    ...
    router.post('/templates', async (req, res) => {
        try {
            const companyId = req.user?.company_id;
            const { name, equipment_type, test_type, frequency_days, parameters, compliance_standard } = req.body;
            const result = await (0, database_js_1.query)(`INSERT INTO test_templates (
