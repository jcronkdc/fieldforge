...
exports.createDrawingRouter = createDrawingRouter;
const express_1 = require("express");
const auth_js_1 = require("../../middleware/auth.js");
const database_js_1 = require("../../database.js");
const auditLog_js_1 = require("../../middleware/auditLog.js");
const zod_1 = require("zod");
function createDrawingRouter() {
    const router = (0, express_1.Router)();
    // Apply authentication to all routes
    router.use(auth_js_1.authenticateRequest);
    // Validation schemas
    const DrawingUploadSchema = zod_1.z.object({
        name: zod_1.z.string().min(1),
        file_data: zod_1.z.string(), // Base64 encoded file
        file_type: zod_1.z.string(),
        sheet_number: zod_1.z.string(),
        revision: zod_1.z.string(),
        discipline: zod_1.z.enum(['architectural', 'structural', 'electrical', 'mechanical', 'civil', 'other']),
        project_id: zod_1.z.number().optional(),
        tags: zod_1.z.array(zod_1.z.string()).optional()
    });
    const AnnotationSchema = zod_1.z.object({
        type: zod_1.z.enum(['comment', 'measurement', 'markup']),
        x: zod_1.z.number(),
        y: zod_1.z.number(),
    ...
    router.get('/:id', async (req, res) => {
        ...
        GROUP BY d.id, p.name, u.raw_user_meta_data`, [id, company_id]);
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const { name, file_data, file_type, sheet_number, revision, discipline, project_id, tags } = validation.data;
            ...
            if (!projectId) {
                const projectResult = await (0, database_js_1.query)('SELECT id FROM projects WHERE company_id = $1 ORDER BY created_at DESC LIMIT 1', [company_id]);
                projectId = projectResult.rows[0]?.id;
            }
            const result = await (0, database_js_1.query)(`INSERT INTO drawings (
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            values.push(id);
            const result = await (0, database_js_1.query)(`UPDATE drawings SET ${updates.join(', ')}
        WHERE id = $${paramIndex}
        AND project_id IN (SELECT id FROM projects WHERE company_id = $${paramIndex + 1})
        RETURNING *`, [...values, company_id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Drawing not found' });
            }
            res.json(result.rows[0]);
    ...
    router.delete('/:id', async (req, res) => {
        try {
            ...
            const result = await (0, database_js_1.query)(`DELETE FROM drawings
        WHERE id = $1
        AND project_id IN (SELECT id FROM projects WHERE company_id = $2)
        RETURNING id`, [id, company_id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Drawing not found' });
            }
    ...
    router.post('/:id/annotations', async (req, res) => {
        try {
            ...
            const drawingCheck = await (0, database_js_1.query)(`SELECT 1 FROM drawings d
        JOIN projects p ON d.project_id = p.id
        WHERE d.id = $1 AND p.company_id = $2`, [drawing_id, company_id]);
            if (drawingCheck.rows.length === 0) {
                return res.status(404).json({ error: 'Drawing not found' });
            }
            const annotation = validation.data;
            const result = await (0, database_js_1.query)(`INSERT INTO drawing_annotations (
    ...
    router.get('/:id/annotations', async (req, res) => {
        try {
            ...
            const result = await (0, database_js_1.query)(`SELECT da.*, u.raw_user_meta_data->>'full_name' as created_by_name
        ...
        ORDER BY da.created_at DESC`, [drawing_id, company_id]);
            res.json(result.rows);
        }
        catch (error) {
    ...
    router.put('/annotations/:id', async (req, res) => {
        try {
            ...
            values.push(id);
            values.push(user_id);
            const result = await (0, database_js_1.query)(`UPDATE drawing_annotations SET ${updates.join(', ')}
        WHERE id = $${paramIndex}
    ...
    router.delete('/annotations/:id', async (req, res) => {
        try {
            ...
            const result = await (0, database_js_1.query)(`DELETE FROM drawing_annotations
