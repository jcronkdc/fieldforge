...
function createOperationsRouter() {
    ...
    router.post('/daily-reports', async (req, res) => {
        try {
            ...
            const companyId = req.user?.company_id;
            const { report_date, project_id, weather_conditions, work_summary, crew_count, crew_details, activities_completed, equipment_used, materials_used, safety_incidents, safety_notes, delays, photos, tomorrow_plan, additional_notes } = req.body;
    ...
    router.put('/daily-reports/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const { weather_conditions, work_summary, crew_count, crew_details, activities_completed, equipment_used, materials_used, safety_incidents, safety_notes, delays, photos, tomorrow_plan, additional_notes } = req.body;
            const result = await (0, database_js_1.query)(`UPDATE daily_reports 
        SET 
    ...
    // Submit daily report
    router.put('/daily-reports/:id/submit', async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const result = await (0, database_js_1.query)(`UPDATE daily_reports 
    ...
    router.put('/daily-reports/:id/approve', async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
        ...
        RETURNING *`, [id, userId, approval_notes]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
    ...
    router.get('/daily-reports/summary', async (req, res) => {
        try {
            const companyId = req.user?.company_id;
      ...
      `;
            ...
            const summaryResult = await (0, database_js_1.query)(queryText, params);
            // Get weather impact summary
            const weatherResult = await (0, database_js_1.query)(`SELECT 
        ...
        GROUP BY weather_conditions->>'work_impact'`, params);
            res.json({
                success: true,
                data: {
    ...
    router.get('/equipment-utilization', async (req, res) => {
        try {
            ...
            const result = await (0, database_js_1.query)(`WITH equipment_data AS (
          SELECT 
            report_date,
        ...
        ORDER BY total_hours DESC`, [companyId, start_date, end_date]);
            res.json({
                success: true,
                data: result.rows
            });
    ...
    router.get('/crew-productivity', async (req, res) => {
        try {
            const companyId = req.user?.company_id;
            const { start_date, end_date, project_id } = req.query;
            const result = await (0, database_js_1.query)(`WITH productivity_data AS (
          SELECT 
            dr.report_date,
            dr.crew_count,
        ...
        ORDER BY EXTRACT(DOW FROM report_date)`, [companyId, start_date, end_date, project_id]);
            res.json({
                success: true,
                data: result.rows
            });
