...
async function createDemoProject() {
    ...
    try {
        ...
        const companyResult = await (0, database_js_1.query)(`
    ...
    `);
        ...
        for (const user of users) {
            const userResult = await (0, database_js_1.query)(`
    ...
    `, [result.company.id, result.users[0].id]); // Sarah Chen as PM
      ...
      `, [result.project.id, assignment.user.id, assignment.role, assignment.permissions]);
        ...
        for (const incident of safetyData) {
            const incidentResult = await (0, database_js_1.query)(`
      ...
      `, [
        ...
        for (const equip of equipmentData) {
            const equipResult = await (0, database_js_1.query)(`
      ...
      `, [
        ...
        for (const inspection of inspections) {
            const inspectionResult = await (0, database_js_1.query)(`
      ...
      `, [
        ...
        const dailyReportResult = await (0, database_js_1.query)(`
    ...
    `, [result.project.id, result.company.id, result.users[1].id]); // Field Supervisor
        ...
        for (const doc of documents) {
            const docResult = await (0, database_js_1.query)(`
      ...
      `, [
        ...
        if (env.DAILY_API_KEY) {
            try {
                ...
                if (dailyResponse.ok) {
                    ...
                    const roomResult = await (0, database_js_1.query)(`
        ...
        const conversationResult = await (0, database_js_1.query)(`
