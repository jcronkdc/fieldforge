...
exports.buildDiscordTurnPrompt = buildDiscordTurnPrompt;
exports.buildDiscordWarning = buildDiscordWarning;
exports.buildDiscordTimeout = buildDiscordTimeout;
exports.buildEmailTurnPrompt = buildEmailTurnPrompt;
exports.buildEmailWarning = buildEmailWarning;
exports.buildEmailTimeout = buildEmailTimeout;
exports.buildSmsTurnPrompt = buildSmsTurnPrompt;
exports.buildSmsWarning = buildSmsWarning;
exports.buildSmsTimeout = buildSmsTimeout;
...
function buildDiscordTurnPrompt(input) {
...
function buildDiscordWarning(input) {
...
function buildDiscordTimeout(input) {
    return {
...
function buildEmailTurnPrompt(input) {
    const countdown = formatCountdown(input.dueAt);
    const dueLabel = dateFormatter.format(input.dueAt);
...
function buildEmailWarning(input) {
    const countdown = formatDuration(input.remainingMs);
...
function buildEmailTimeout(input) {
    ...
    <div style="font-family:Inter,system-ui; background:#020617; padding:24px; color:#e2e8f0;">
      <div style="max-width:520px; margin:0 auto; background:rgba(122,53,191,0.4); border:1px solid rgba(217,70,239,0.45); border-radius:16px; padding:24px;">
        ...
        <p style="margin:0 0 16px; font-size:15px; color:#f8fafc;">${escapeHtml(input.prompt)}</p>
        ${input.aiFill
        ? `<p style="margin:0 0 16px; font-size:14px;">AI co-host filled the blank with:</p><blockquote style="margin:0 0 16px; font-size:16px; color:#f8fafc;">${escapeHtml(input.aiFill)}</blockquote>`
