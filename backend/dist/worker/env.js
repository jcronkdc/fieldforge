...
function loadEnv() {
    ...
    const { DATABASE_URL, SUPABASE_URL, SUPABASE_SERVICE_KEY, POSTHOG_API_KEY, AI_FALLBACK_MODEL, AI_FALLBACK_API_KEY, AI_PROVIDER_URL, AI_PROVIDER_API_KEY, AI_PROVIDER_MODEL, ANTHROPIC_API_KEY, OPENAI_API_KEY, OPENWEATHER_API_KEY, NOTIFY_WEBHOOK_DISCORD, NOTIFY_SENDGRID_API_KEY, NOTIFY_EMAIL_FROM, NOTIFY_TWILIO_SID, NOTIFY_TWILIO_AUTH_TOKEN, NOTIFY_TWILIO_FROM, APP_BASE_URL, ABLY_API_KEY, DAILY_API_KEY, STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, RESEND_API_KEY, TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER, SIREN_API_ENDPOINT, } = process.env;
    if (!DATABASE_URL) {
        if (process.env.NODE_ENV === "production") {
