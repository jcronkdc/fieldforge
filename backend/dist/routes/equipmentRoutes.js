...
function createEquipmentRouter() {
    ...
    router.get('/inventory', auth_js_1.authenticateRequest, async (req, res) => {
        try {
    ...
    router.get('/inventory/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const { id } = req.params;
        ...
        WHERE e.id = $1
        GROUP BY e.id, p.name, u.email, u.raw_user_meta_data`, [id]);
    ...
    // Add new equipment
    router.post('/inventory', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ error: 'Authentication required' });
            }
            const { equipment_type, manufacturer, model, serial_number, asset_tag, purchase_date, purchase_price, condition = 'new', maintenance_interval_days, location_description, notes } = req.body;
            if (!equipment_type) {
            ...
            if (serial_number || asset_tag) {
                const dupCheck = await (0, database_js_1.query)(`SELECT id FROM equipment_inventory 
           WHERE serial_number = $1 OR asset_tag = $2`, [serial_number, asset_tag]);
            ...
            const result = await (0, database_js_1.query)(`INSERT INTO equipment_inventory 
         ...
         RETURNING *`, [equipment_type, manufacturer, model, serial_number, asset_tag,
                purchase_date, purchase_price, condition, maintenance_interval_days,
    ...
    // Update equipment
    router.put('/inventory/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            const updateFields = [];
    ...
    // Check out equipment
    router.post('/checkout/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            ...
            const availability = await (0, database_js_1.query)('SELECT status, assigned_to FROM equipment_inventory WHERE id = $1', [id]);
            ...
            try {
                ...
                await (0, database_js_1.query)(`UPDATE equipment_inventory 
           SET status = 'in use', 
               assigned_to = $1, 
               current_project_id = $2
           WHERE id = $3`, [userId, project_id, id]);
                // Create usage log
                const logResult = await (0, database_js_1.query)(`INSERT INTO equipment_usage_logs 
           ...
           RETURNING *`, [id, project_id, userId, expected_return_date, notes]);
    ...
    // Check in equipment
    router.post('/checkin/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            ...
            const usageCheck = await (0, database_js_1.query)(`SELECT id, checked_out_at FROM equipment_usage_logs 
         WHERE equipment_id = $1 AND checked_out_by = $2 AND checked_in_at IS NULL
         ORDER BY checked_out_at DESC LIMIT 1`, [id, userId]);
            ...
            try {
                // Update usage log
                await (0, database_js_1.query)(`UPDATE equipment_usage_logs 
           SET checked_in_at = NOW(), 
               hours_used = $1, 
               condition_in = $2, 
               notes = COALESCE(notes || ' | Check-in: ' || $3, $3)
           WHERE id = $4`, [actualHours, condition_in, notes, usageLogId]);
                // Update equipment
           ...
           WHERE id = $4`, [status, condition_in || 'good', actualHours, id]);
                // Create maintenance request if needed
                if (maintenance_needed) {
                    await (0, database_js_1.query)(`INSERT INTO maintenance_requests 
             (equipment_id, requested_by, reason, priority, status)
             VALUES ($1, $2, $3, $4, 'pending')`, [id, userId, notes || 'Maintenance requested at check-in', 'medium']);
                }
                await (0, database_js_1.query)('COMMIT');
                res.json({
    ...
    router.get('/scan/:code', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const { code } = req.params;
            const result = await (0, database_js_1.query)(`SELECT * FROM equipment_inventory 
         WHERE serial_number = $1 OR asset_tag = $1`, [code]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Equipment not found' });
            }
            res.json(result.rows[0]);
    ...
    router.post('/maintenance/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            ...
            const result = await (0, database_js_1.query)(`INSERT INTO maintenance_requests 
         ...
         RETURNING *`, [id, userId, scheduled_date, description, estimated_hours, assigned_to]);
    ...
    router.get('/maintenance/history/:id', auth_js_1.authenticateRequest, async (req, res) => {
        try {
            const { id } = req.params;
            const result = await (0, database_js_1.query)(`SELECT 
          mr.*,
         ...
         ORDER BY mr.created_at DESC`, [id]);
            res.json(result.rows);
        }
        catch (error) {
    ...
    router.get('/reports/utilization', auth_js_1.authenticateRequest, async (req, res) => {
        try {
      ...
      `;
            const result = await (0, database_js_1.query)(reportQuery, [start_date, end_date, project_id]);
            res.json({
                period: {
