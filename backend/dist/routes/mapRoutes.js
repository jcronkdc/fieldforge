...
const equipmentSchema = zod_1.z.object({
    ...
    status: zod_1.z.enum(['active', 'idle', 'maintenance']),
    operator_id: zod_1.z.string().uuid().optional()
});
const crewLocationSchema = zod_1.z.object({
    user_id: zod_1.z.string().uuid(),
    position: positionSchema,
    zone: zod_1.z.string(),
    status: zod_1.z.enum(['active', 'break', 'offline'])
});
const safetyZoneSchema = zod_1.z.object({
    name: zod_1.z.string(),
    type: zod_1.z.enum(['restricted', 'caution', 'safe']),
...
function createMapRouter() {
    ...
    router.get('/equipment', async (req, res) => {
        try {
         ...
         GROUP BY e.id, e.name, e.type, e.status, ep.position, ep.updated_at, u.name`, [project_id]);
    ...
    router.put('/equipment/:equipmentId/position', async (req, res) => {
        try {
            const userId = req.user?.id;
            ...
            const equipmentCheck = await (0, database_js_1.query)(`SELECT e.id, e.project_id 
         FROM equipment e
         JOIN project_members pm ON e.project_id = pm.project_id
         WHERE e.id = $1 AND pm.user_id = $2`, [equipmentId, userId]);
            ...
            await (0, database_js_1.query)(`INSERT INTO equipment_positions (equipment_id, position, updated_by)
         VALUES ($1, $2, $3)`, [equipmentId, JSON.stringify(validatedPosition), userId]);
            // Also insert into history table
            await (0, database_js_1.query)(`INSERT INTO equipment_position_history (equipment_id, position, recorded_by)
         VALUES ($1, $2, $3)`, [equipmentId, JSON.stringify(validatedPosition), userId]);
            auditLogger_js_1.auditLogger.log('equipment_position_updated', userId, {
    ...
    router.put('/crew/location', async (req, res) => {
        try {
            const userId = req.user?.id;
            const validatedData = crewLocationSchema.parse(req.body);
            // User can only update their own location unless they're a supervisor
            const userCheck = await (0, database_js_1.query)('SELECT role FROM users WHERE id = $1', [userId]);
            const canUpdateOthers = ['admin', 'supervisor', 'safety_officer'].includes(userCheck.rows[0]?.role);
            ...
            await (0, database_js_1.query)(`INSERT INTO crew_locations (user_id, position, zone, status, updated_at)
         VALUES ($1, $2, $3, $4, NOW())
         ON CONFLICT (user_id)
    ...
    router.get('/zones', async (req, res) => {
        try {
            ...
            const zonesResult = await (0, database_js_1.query)(`SELECT * FROM safety_zones 
         WHERE project_id = $1 
    ...
    router.post('/zones', async (req, res) => {
        try {
            ...
            const userCheck = await (0, database_js_1.query)(`SELECT u.role 
         FROM users u
         JOIN project_members pm ON u.id = pm.user_id
         WHERE u.id = $1 AND pm.project_id = $2`, [userId, project_id]);
            const canManageZones = ['admin', 'safety_officer', 'supervisor'].includes(userCheck.rows[0]?.role);
            if (!canManageZones) {
                return res.status(403).json({
                    success: false,
            ...
            const result = await (0, database_js_1.query)(`INSERT INTO safety_zones 
         ...
         RETURNING id`, [
            ...
            auditLogger_js_1.auditLogger.log('safety_zone_created', userId, {
                project_id,
    ...
    router.patch('/zones/:zoneId/toggle', async (req, res) => {
        try {
            ...
            // Verify permissions
            const zoneCheck = await (0, database_js_1.query)(`SELECT sz.active, u.role
         FROM safety_zones sz
         JOIN project_members pm ON sz.project_id = pm.project_id
         JOIN users u ON pm.user_id = u.id
         WHERE sz.id = $1 AND u.id = $2`, [zoneId, userId]);
            ...
            const canManageZones = ['admin', 'safety_officer', 'supervisor'].includes(zoneCheck.rows[0].role);
            if (!canManageZones) {
                return res.status(403).json({
                    success: false,
            ...
            await (0, database_js_1.query)('UPDATE safety_zones SET active = $1, updated_at = NOW() WHERE id = $2', [newState, zoneId]);
            auditLogger_js_1.auditLogger.log('safety_zone_toggled', userId, {
                zone_id: zoneId,
                active: newState
            });
    ...
    router.get('/stats', async (req, res) => {
        try {
            ...
            const [equipment, crew, zones] = await Promise.all([
                (0, database_js_1.query)(`SELECT status, COUNT(*) as count 
           FROM equipment 
           WHERE project_id = $1 
           GROUP BY status`, [project_id]),
                (0, database_js_1.query)(`SELECT cl.status, COUNT(*) as count
           FROM crew_locations cl
           JOIN users u ON cl.user_id = u.id
           JOIN project_members pm ON u.id = pm.user_id
           WHERE pm.project_id = $1
             AND cl.updated_at > NOW() - INTERVAL '30 minutes'
           GROUP BY cl.status`, [project_id]),
                (0, database_js_1.query)(`SELECT type, COUNT(*) as count
           FROM safety_zones
