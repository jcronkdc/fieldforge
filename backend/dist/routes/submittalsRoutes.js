...
exports.createSubmittalsRouter = createSubmittalsRouter;
const express_1 = require("express");
const database_js_1 = require("../database.js");
const zod_1 = require("zod");
const auditLogger_js_1 = require("../middleware/auditLogger.js");
// Validation schemas
const createSubmittalSchema = zod_1.z.object({
    project_id: zod_1.z.string().uuid(),
    submittal_number: zod_1.z.string(),
    title: zod_1.z.string().min(1),
    spec_section: zod_1.z.string().min(1),
    description: zod_1.z.string().optional(),
    priority: zod_1.z.enum(['low', 'medium', 'high', 'critical']).optional(),
    required_date: zod_1.z.string().optional(),
    attachments: zod_1.z.array(zod_1.z.string()).optional()
});
const updateSubmittalSchema = zod_1.z.object({
    status: zod_1.z.enum(['draft', 'submitted', 'under_review', 'approved', 'rejected', 'revise_resubmit']).optional(),
    reviewer_comments: zod_1.z.string().optional(),
    approved_date: zod_1.z.string().optional(),
    attachments: zod_1.z.array(zod_1.z.string()).optional()
});
function createSubmittalsRouter() {
    ...
    router.get('/', async (req, res) => {
      ...
      `;
            ...
            await (0, auditLogger_js_1.auditLog)(userId, 'view_submittals', {
    ...
    router.get('/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const result = await (0, database_js_1.query)(`SELECT s.*, u.raw_user_meta_data->>'full_name' as submitted_by_name
         FROM submittals s
         LEFT JOIN auth.users u ON s.submitted_by = u.id
         WHERE s.id = $1`, [id]);
            ...
            await (0, auditLogger_js_1.auditLog)(userId, 'view_submittal', { submittal_id: id });
            res.json({
                success: true,
                data: result.rows[0]
            });
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const result = await (0, database_js_1.query)(`INSERT INTO submittals (
        ...
        RETURNING *`, [
            ...
            await (0, auditLogger_js_1.auditLog)(userId, 'create_submittal', {
    ...
    router.patch('/:id', async (req, res) => {
      ...
      `;
            ...
            await (0, auditLogger_js_1.auditLog)(userId, 'update_submittal', {
                submittal_id: id,
    ...
    router.delete('/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const result = await (0, database_js_1.query)('DELETE FROM submittals WHERE id = $1 RETURNING *', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Submittal not found'
            ...
            await (0, auditLogger_js_1.auditLog)(userId, 'delete_submittal', {
                submittal_id: id,
