...
exports.getProjectWeather = getProjectWeather;
exports.analyzeWeatherImpact = analyzeWeatherImpact;
...
try {
    if (process.env.ANTHROPIC_API_KEY) {
        anthropicClient = new sdk_1.default({ apiKey: process.env.ANTHROPIC_API_KEY });
    }
}
catch (error) {
 ...
 */
async function getCurrentWeather(zipCode, country = 'US') {
    if (!WEATHER_API_KEY) {
    ...
    try {
        ...
        const geoResponse = await axios_1.default.get(`${GEOCODING_API_BASE}/zip`, {
            params: {
                zip: `${zipCode},${country}`,
        ...
        // Get current weather
        const weatherResponse = await axios_1.default.get(`${WEATHER_API_BASE}/weather`, {
            params: {
                lat,
                lon,
                appid: WEATHER_API_KEY,
 ...
 */
async function getWeatherForecast(zipCode, country = 'US', days = 5) {
    if (!WEATHER_API_KEY) {
        console.warn('[Weather] OPENWEATHER_API_KEY not configured');
    ...
    try {
        ...
        const forecastResponse = await axios_1.default.get(`${WEATHER_API_BASE}/forecast`, {
            params: {
                lat,
                lon,
                appid: WEATHER_API_KEY,
 ...
 */
async function getProjectWeather(projectId, includeForecast = false) {
    try {
 ...
 */
async function analyzeWeatherImpact(zipCode, projectId, activities = []) {
    try {
        // Get weather data
        let forecast;
        if (projectId) {
 ...
 */
function processForecastData(forecastList, days = 5) {
    const dailyData = {};
    // Group by date
 ...
 */
function assessConstructionImpact(weatherData) {
 ...
 */
function assessWorkability(high, low, conditions, windSpeed, precipProb) {
    let score = 100;
 ...
 */
function identifyConstructionConcerns(high, low, conditions, windSpeed, precipProb) {
    const concerns = [];
    if (conditions === 'Thunderstorm')
        concerns.push('⚠️ Lightning hazard - outdoor work prohibited');
    if (precipProb > 50)
        concerns.push('☔ High precipitation chance - plan indoor work');
    if (windSpeed > 15)
        concerns.push('💨 Elevated winds - crane operations may be restricted');
    if (high > 90)
        concerns.push('🌡️ Heat stress risk - increase water breaks');
    if (low < 40)
        concerns.push('❄️ Cold temperatures - protect concrete work');
    if (conditions === 'Rain')
        concerns.push('🌧️ Wet conditions - slip/fall hazards');
    if (conditions === 'Snow')
        concerns.push('🌨️ Snow - equipment operation challenges');
    return concerns;
}
/**
 * Calculate overall workability score
 */
function calculateWorkabilityScore(dailyForecasts) {
 ...
 */
function generateWeatherAlerts(dailyForecasts) {
    const alerts = [];
 ...
 */
function generateConstructionRecommendations(dailyForecasts) {
 ...
 */
function generateFallbackWeather(zipCode, type, days = 5) {
    const baseResponse = {
        location: {
            name: 'Location',
            zipCode
 ...
 */
function generateFallbackWeatherAnalysis(forecast, activities) {
    return {
