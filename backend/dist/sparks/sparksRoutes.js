...
const sparksRepository_1 = require("./sparksRepository");
const auth_1 = require("../middleware/auth");
const router = (0, express_1.Router)();
const sparksRepo = new sparksRepository_1.SparksRepository();
...
router.get('/user/:userId', auth_1.authenticateRequest, async (req, res) => {
    try {
...
router.post('/purchase', auth_1.authenticateRequest, async (req, res) => {
    try {
        ...
        const purchaseId = await sparksRepo.createPurchase(userId, 'sparks', selectedPackage.priceCents, selectedPackage.sparksAmount + selectedPackage.bonusSparks, packageId, undefined, paymentMethod);
        ...
        // Grant Sparks
        const newBalance = await sparksRepo.addSparks(userId, selectedPackage.sparksAmount + selectedPackage.bonusSparks, 'purchase', `Purchased ${selectedPackage.displayName}`, 'purchase', purchaseId);
        // Complete purchase
...
// Subscribe to tier
router.post('/subscribe', auth_1.authenticateRequest, async (req, res) => {
    try {
        ...
        const subscriptionId = await sparksRepo.upsertSubscription(userId, tierId, billingCycle);
        // Grant initial monthly Sparks
        if (selectedTier.monthlySparks > 0) {
            await sparksRepo.addSparks(userId, selectedTier.monthlySparks, 'subscription', 'Monthly subscription Sparks', 'subscription', subscriptionId);
        }
        res.json({
            success: true,
            subscriptionId,
...
// Cancel subscription
router.post('/cancel-subscription', auth_1.authenticateRequest, async (req, res) => {
    try {
...
// Use Sparks
router.post('/use', auth_1.authenticateRequest, async (req, res) => {
    try {
        ...
        const success = await sparksRepo.useSparks(userId, amount, feature, referenceType, referenceId);
        if (!success) {
...
// Claim daily bonus
router.post('/claim-daily', auth_1.authenticateRequest, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/usage-stats/:userId', auth_1.authenticateRequest, async (req, res) => {
    try {
        const { userId } = req.params;
        // Verify user is accessing their own data
        if (req.user?.id !== userId) {
...
router.post('/referral', auth_1.authenticateRequest, async (req, res) => {
    try {
