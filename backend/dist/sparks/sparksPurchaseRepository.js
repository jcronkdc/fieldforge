...
exports.getSparksPackages = getSparksPackages;
exports.createSparksCheckoutSession = createSparksCheckoutSession;
...
async function completePurchase(data) {
    ...
    try {
        ...
        const purchaseResult = await client.query(`UPDATE sparks_purchases 
       ...
       AND package_id = $3 
       AND status = 'pending'
       RETURNING *`, [data.paymentIntentId, data.userId, data.packageId]);
       ...
       SET mytha_coins = mytha_coins + $1,
           updated_at = NOW()
       WHERE user_id = $2`, [purchase.total_sparks, data.userId]);
        // Record in transaction ledger
        await client.query(`INSERT INTO mythacoin_transactions (
...
async function redeemGiftCode(data) {
    ...
    try {
        ...
        const codeResult = await client.query(`SELECT * FROM sparks_gift_codes 
       WHERE UPPER(code) = UPPER($1)
       AND redeemed_by IS NULL
       AND (valid_until IS NULL OR valid_until > NOW())
       AND redemption_count < max_redemptions
       FOR UPDATE`, [data.code]);
        if (codeResult.rows.length === 0) {
            return { success: false, error: "Invalid or expired gift code" };
        }
        const giftCode = codeResult.rows[0];
        // Mark as redeemed
        await client.query(`UPDATE sparks_gift_codes
       SET redeemed_by = $1,
           redeemed_at = NOW(),
           redemption_count = redemption_count + 1
       WHERE id = $2`, [data.userId, giftCode.id]);
        // Credit Sparks to user
        await client.query(`UPDATE user_profiles
       SET mytha_coins = mytha_coins + $1,
           updated_at = NOW()
       WHERE user_id = $2`, [giftCode.sparks_amount, data.userId]);
        // Record in transaction ledger
        await client.query(`INSERT INTO mythacoin_transactions (
        user_id, amount, transaction_type, description, metadata
      ) VALUES ($1, $2, $3, $4, $5)`, [
