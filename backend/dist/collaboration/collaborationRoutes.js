...
const collaborationRepository_js_1 = require("./collaborationRepository.js");
const collaborationPublisher_js_1 = require("../realtime/collaborationPublisher.js");
/**
...
function createCollaborationRouter() {
    ...
    router.post("/rooms", async (req, res) => {
        ...
        try {
            ...
            const room = await (0, collaborationRepository_js_1.createCollaborationRoom)(projectId, roomName, createdBy, {
                id: dailyRoom.id,
                name: dailyRoom.name,
                url: dailyRoom.url
            }, conversationId, privacy);
            // Add creator as room participant
            await (0, collaborationRepository_js_1.addRoomParticipant)(room.id, createdBy, req.body.creatorName || 'Room Creator', 'host', {
                canScreenShare: true,
                canRecord: true,
            ...
            await (0, collaborationPublisher_js_1.publishRoomEvent)(room.id, 'room.created', {
    ...
    router.get("/projects/:projectId/rooms", async (req, res) => {
        ...
        try {
            const rooms = await (0, collaborationRepository_js_1.getProjectRooms)(projectId);
            res.json({ rooms });
        }
        catch (error) {
    ...
    router.delete("/rooms/:roomId", async (req, res) => {
        ...
        try {
            ...
            const room = await (0, collaborationRepository_js_1.getCollaborationRoom)(roomId);
            if (!room) {
                return res.status(404).json({ error: "Room not found" });
            }
            // Verify user has permission (creator or project admin)
            // This throws an error if unauthorized
            await (0, collaborationRepository_js_1.endCollaborationRoom)(roomId, userId);
            ...
            await (0, collaborationPublisher_js_1.publishRoomEvent)(roomId, 'room.ended', {
                endedBy: userId
            });
     ...
     */
    router.post("/rooms/:roomId/tokens", async (req, res) => {
        const { roomId } = req.params;
        ...
        try {
            ...
            try {
                await (0, collaborationRepository_js_1.addRoomParticipant)(roomId, userId, userName, permissions.isOwner ? 'host' : 'participant', permissions);
            }
            catch (err) {
                // Participant might already exist, update token instead
                await (0, collaborationRepository_js_1.updateParticipantToken)(roomId, userId, tokenData.token, expiresAt);
            }
            res.json({
                success: true,
    ...
    router.post("/rooms/:roomId/cursor", async (req, res) => {
        ...
        try {
            ...
            await (0, collaborationPublisher_js_1.publishCursorUpdate)(roomId, userId, userName, {
                x,
                y,
                action: action || 'move',
                documentId
