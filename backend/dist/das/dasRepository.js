...
exports.createVotingCycle = createVotingCycle;
exports.getCurrentVotingCycle = getCurrentVotingCycle;
exports.listVotingCycles = listVotingCycles;
exports.getCampaignById = getCampaignById;
exports.listActiveCampaigns = listActiveCampaigns;
exports.updateCampaignMetrics = updateCampaignMetrics;
exports.createAdContent = createAdContent;
exports.getAdContent = getAdContent;
exports.listUserEligibleAds = listUserEligibleAds;
exports.recordEngagement = recordEngagement;
exports.getUserEngagements = getUserEngagements;
exports.getUserPreferences = getUserPreferences;
exports.updateUserPreferences = updateUserPreferences;
exports.distributeRevenue = distributeRevenue;
exports.getUserPayouts = getUserPayouts;
exports.getRevenueStats = getRevenueStats;
exports.submitBrandFeedback = submitBrandFeedback;
...
async function registerBrand(data) {
    ...
    // Create audit entry
    await (0, dasAudit_js_1.createDasAuditEntry)({
...
async function getProposalById(proposalId) {
    const result = await query(`SELECT p.*, b.company_name, b.reputation_score,
     ...
     WHERE p.id = $1
     GROUP BY p.id, b.company_name, b.reputation_score`, [proposalId]);
    return result.rows[0];
}
async function listProposals(cycleId, status) {
    let sql = `
...
async function checkProposalVotingStatus(proposalId) {
    ...
    const statsResult = await query(`SELECT 
     ...
     LEFT JOIN das_votes v ON p.id = v.proposal_id
     WHERE p.id = $1
     GROUP BY p.id, vc.min_votes_required, vc.approval_threshold`, [proposalId]);
...
async function rejectProposal(proposalId, approvalPercentage, reason) {
    await query(`UPDATE das_proposals 
     SET status = 'rejected',
         ...
         rejection_reason = $3,
         updated_at = NOW()
     WHERE id = $1`, [proposalId, approvalPercentage, reason]);
    // Create audit entry
    await (0, dasAudit_js_1.createDasAuditEntry)({
...
async function listVotingCycles(status) {
    const sql = status
...
async function updateCampaignMetrics(campaignId, metrics) {
    const result = await query(`UPDATE das_campaigns
     SET performance_metrics = performance_metrics || $2,
         ...
         updated_at = NOW()
     WHERE id = $1
     RETURNING *`, [campaignId, JSON.stringify(metrics)]);
    return result.rows[0];
}
// ============================================================================
...
async function getAdContent(contentId) {
    const result = await query(`SELECT ac.*, c.campaign_name, b.company_name
     ...
     JOIN das_brands b ON c.brand_id = b.id
     WHERE ac.id = $1 AND ac.is_active = true`, [contentId]);
    return result.rows[0];
}
...
async function getDashboardStats(userId) {
    ...
    if (userId) {
        ...
        (SELECT opt_in_ads FROM das_user_preferences WHERE user_id = $1) as opted_in`, [userId]);
