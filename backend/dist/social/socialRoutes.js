...
exports.createSocialRouter = createSocialRouter;
const express_1 = __importDefault(require("express"));
const socialRepository_js_1 = require("./socialRepository.js");
function createSocialRouter() {
    ...
    router.get("/bookworms", async (req, res) => {
        ...
        try {
            ...
            const [items, stats] = await Promise.all([
                (0, socialRepository_js_1.listBookworms)(userId, limit),
                (0, socialRepository_js_1.getConnectionStats)(userId),
            ]);
    ...
    router.delete("/bookworms/:friendId", async (req, res) => {
        ...
        try {
            await (0, socialRepository_js_1.removeBookworm)(userId, req.params.friendId);
            res.status(204).send();
        }
        catch (error) {
    ...
    router.get("/requests", async (req, res) => {
        ...
        try {
            const requests = await (0, socialRepository_js_1.listConnectionRequests)(userId, direction);
    ...
    router.post("/requests", async (req, res) => {
        ...
        try {
            ...
            if (!resolvedTargetId && typeof targetUsername === "string") {
                const profile = await (0, socialRepository_js_1.lookupProfileByUsername)(targetUsername);
                if (!profile) {
            ...
            const request = await (0, socialRepository_js_1.createConnectionRequest)({
                requesterId,
    ...
    router.post("/requests/:requestId/respond", async (req, res) => {
        ...
        try {
            const result = await (0, socialRepository_js_1.respondToRequest)({
    ...
    router.get("/profiles/lookup", async (req, res) => {
        ...
        try {
            const profile = await (0, socialRepository_js_1.lookupProfileByUsername)(username);
            if (!profile) {
                return res.status(404).json({ error: "User not found" });
            }
            res.json({ profile });
